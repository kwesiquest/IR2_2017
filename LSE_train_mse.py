...
import sys
import copy 
from LSE_mse import LSE as LSE
import operator
...
trainWriter = tf.summary.FileWriter(next_folder('train/mse'))
testWriter = tf.summary.FileWriter(next_folder('test/mse'))
...
#        print("loss:", sess.run(loss, feed_dict={ngrams_placeholder: ngrams, documents_placeholder: similar, dissimilar_placeholder: diss}))
        ...
        if i % 50 == 0 or i == TRAIN_STEPS:
            direc = os.getcwd()
            path = saver.save(sess, direc + '/saves/model_mse.ckpt')
            print('Saved in ', path)
